...
mkdir -p $S/config $S/userdata/12345678/config $HOME/.config/yet-another-creature-collector-game-launcher
...
printf '{"install_dir":"%s"}' "$1" > $HOME/.config/yet-another-creature-collector-game-launcher/settings.json
if [ "$3" = "--with-shortcut" ]; then /target/yaccgl-bin steam add --no-artwork >/dev/null; fi
Xvfb :9 -screen 0 1280x800x24 >/dev/null 2>&1 &
sleep 1
DISPLAY=:9 dbus-run-session -- /target/debug/yet-another-creature-collector-game-launcher >/tmp/gui.log 2>&1 &
sleep 8
